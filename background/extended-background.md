...
## Black-Box, Grey-Box, and White-Box Fuzzing

Fuzzing approaches can also be distinguished by the amount of information they
obtain from the target.
...
<th>Examples in AI-system testing</th>
</tr>
</thead>
<tbody>
...
<td>Uses only externally observable inputs and outputs.</td>
<td>Prediction labels, confidence scores, task outputs, response text, runtime behavior, or externally visible safety violations.</td>
</tr>

<tr>
<td><strong>Grey-box</strong></td>
<td>Uses lightweight internal feedback without requiring full program semantics.</td>
<td>Neuron activation, model coverage, API coverage, compiler-path coverage, state novelty, trajectory coverage, or runtime instrumentation.</td>
</tr>

<tr>
<td><strong>White-box</strong></td>
<td>Uses deeper internal information about the model or implementation.</td>
<td>Gradients, symbolic constraints, source-level information, internal execution states, operator constraints, model structure, or compiler intermediate representations.</td>
</tr>
...
These modes are not always sharply separated. A single AI fuzzer may combine
external behavioral observations with internal coverage, gradients,
constraints, or execution traces.
...
Because these test artifacts are often semantically constrained, effective
fuzzing must balance two competing goals:

- **validity** — the test should remain executable, meaningful, or realistic;
- **diversity** — the test should explore unusual or failure-inducing behavior.

Overly weak constraints may produce invalid tests that fail before reaching
deep target behavior. Overly strict constraints may prevent exploration of rare
but important edge cases.

---

## Feedback in AI-System Fuzzing

Feedback determines which generated tests are considered useful for future
exploration.

Depending on the target, feedback may include:

- code or branch coverage;
- neuron or activation coverage;
- layer or region coverage;
- API or operator coverage;
- compiler-path or pass coverage;
- numerical divergence;
- state or trajectory novelty;
- prediction difference;
- safety-rule violation;
- behavioral diversity;
- exception type;
- runtime performance; or
- domain-specific objective scores.

No single feedback signal is suitable for every AI-system layer.

For example:

- model-level fuzzers may prioritize tests that activate new neurons or expose
  new behavioral regions;
- framework fuzzers may retain tests that exercise new APIs, operator
  combinations, or implementation paths;
- compiler fuzzers may target optimization passes, intermediate
  representations, or backend behaviors;
- system-level fuzzers may prioritize novel scenarios, trajectories, unsafe
  states, or policy failures.

This target dependence motivates the separate taxonomy dimensions used in the
survey.

---

## Failure Detection Beyond Crashes

A central difference between conventional software fuzzing and AI-system
fuzzing is that an AI system may fail without crashing.

Relevant failure signals include:

- incorrect prediction;
- changed or unstable label;
...
- perception failure;
- interaction failure; and
- other domain-specific behavioral errors.

Crash and exception signals remain useful for executable AI software
components, but they capture only part of the failure space.

This is why the survey separately analyzes:

- **Oracle Type** — the high-level principle used to recognize a failure;
- **Oracle Construction** — how that principle is implemented; and
- **Failure Type** — the abnormal behavior ultimately exposed.

The extended oracle and failure analyses are available on the
...
## Cross-Layer Nature of AI-System Failures

AI-enabled applications consist of multiple interacting layers:
...
Input / Scenario / Prompt
   ...
   Compiler / Backend
            ↓
 Runtime / Hardware / System
            ↓
 Application-Level Behavior
```

A failure may be observed at one layer but caused by another.

Examples include:

- a model prediction error caused by invalid preprocessing;
- a numerical inconsistency caused by a framework operator;
- a wrong output introduced by a compiler optimization;
- an unsafe system action caused by interactions among perception, planning,
  control logic, and environment state.

This distinction motivates separating the **Testing Target** from the
**System Level** at which behavior is observed.

The full definitions are available on the
[Extended Taxonomy Definitions page]({{ site.baseurl }}/background/taxonomy-definitions.html).
